...
const SingleRide = () => {
    ...
    useEffect(() => {
        ...
        let check = async () => {
            ...
            if (data.status == 'activityInS3' && data.progress <= 1 && !process.env.ENVIRONMENT) {
                generateVideo();
            }
...
return (
    <div className='single-ride'>
        ...
        <div>
            ...
            {videoInS3 == 'videoInS3' && <Link to={`/visualize/${id}?custom=${custom}`}>Play Ride</Link>}
